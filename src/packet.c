...
static parseResult packet_recv_buf(net_buffer_t *pak, char **buf,
                                   unsigned long *size);
static parseResult packet_send_buf(net_buffer_t *pak, const char *buf,
                                   unsigned long size);

parseResult packet_realloc(net_buffer_t *pak, size_t capacity)
{
    if (pak->size > capacity)
        return PACKET_ERROR;
...
parseResult packet_recv_packet(net_buffer_t *pak, net_buffer_t *buf)
{
    ...
    if ((result = packet_recv_buf(pak, &(buf->buffer), &capacity)) !=
        PACKET_SUCCESS)
        return result;
    buf->size = 0;
                                   ...
                                   unsigned long *size)
{
    long int length;
    parseResult parsed;
    if ((parsed = packet_recv_i32(pak, &length)) != PACKET_SUCCESS)
        return parsed;
                                   ...
                                   unsigned long size)
{
    ...
    if (size >= (size_t)LONG_MAX)
        return PACKET_ERROR;
    if ((parsed = packet_send_i32(pak, size)) != PACKET_SUCCESS)
        return parsed;
