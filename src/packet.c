...
parseResult packet_send_i16(packet_t *pak, short int n)
{
    ...
    if ((pak->size + sizeof(short int) > pak->capacity) &&
        (parsed = packet_realloc(pak, pak->capacity + sizeof(short int))) !=
            PACKET_SUCCESS)
        return parsed;
...
parseResult packet_send_i32(packet_t *pak, long int n)
{
    ...
    if ((pak->size + sizeof(long int) > pak->capacity) &&
        (parsed = packet_realloc(pak, pak->capacity + sizeof(long int))) !=
            PACKET_SUCCESS)
        return parsed;
...
parseResult packet_recv_str(packet_t *pak, char **res)
{
    short int length;
    parseResult parsed;
    if ((parsed = packet_recv_i16(pak, &length)) != PACKET_SUCCESS)
        return parsed;
...
parseResult packet_send_str(packet_t *pak, const char *str)
{
    short int length;
    parseResult parsed;
    if (strlen(str) >= (size_t)SHRT_MAX)
        return PACKET_ERROR;
    length = strlen(str) + 1;
    if ((parsed = packet_send_i16(pak, length)) != PACKET_SUCCESS)
        return parsed;
    if ((pak->size + length > pak->capacity) &&
        (parsed = packet_realloc(pak, pak->capacity + length)) !=
            PACKET_SUCCESS)
        return parsed;
