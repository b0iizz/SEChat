...
txpResult txp_init()
{
    if ((terminal_fd = open("/dev/tty", O_RDWR | O_NONBLOCK)) < 0)
        return TXP_ERROR;
    return TXP_SUCCESS;
}
txpResult txp_exit()
{
    if (close(terminal_fd) < 0)
        return TXP_ERROR;
    return TXP_SUCCESS;
}
...
{
    return tcsetattr(terminal_fd, TCSAFLUSH, save) < 0 ? TXP_ERROR :
                                                         TXP_SUCCESS;
}
...
{
    ssize_t status;
    if ((status = read(terminal_fd, buffer, blen)) < 0)
        return TXP_ERROR;
    *nread = status;
    if (status == 0)
...
txpResult txp_flush()
{
    if (tcflush(terminal_fd, TCIFLUSH) < 0)
        return TXP_ERROR;
    return TXP_SUCCESS;
}
