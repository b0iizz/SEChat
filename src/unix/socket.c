                           ...
                           const char *serviceport, addrinfo_t *hints)
{
    ...
    if (result != 0) {
        return sxp_map_eai_error(result, errno);
    }
    return SXP_SUCCESS;
}
...
{
    ssize_t read;
    if (!sock || !data || !num_read)
        return SXP_ERROR_INVAL;
    if ((read = recv(*sock, data, size, 0)) < 0)
        return sxp_map_error(errno);
    *num_read = read;
    return SXP_SUCCESS;
}
                   ...
                   size_t sxpcount, int timeout)
{
    ssize_t result;
    if (!sxpcount)
        return SXP_TRY_AGAIN;
    if (!sxps)
        return SXP_ERROR_INVAL;
    if ((result = poll(sxps, sxpcount, timeout)) < 0)
        return sxp_map_error(errno);
    if (results)
        *results = result;
...
static sxpResult sxp_map_eai_error(int error, int system_errno)
{
    switch (error) {
    case EAI_MEMORY:
        return SXP_ERROR_MEMORY;
    case EAI_NONAME:
        return SXP_ERROR_UNKNOWN;
    case EAI_SYSTEM:
        return sxp_map_error(system_errno);
    case EAI_AGAIN:
        return SXP_TRY_AGAIN;
    case EAI_BADFLAGS:
    case EAI_FAMILY:
