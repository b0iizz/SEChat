...
netResult netio_connect(const char *hostname, const char *port)
{
  ...
  if ((res = sxp_addrinfo_get(&addresses, hostname, port, &hints)) != SXP_SUCCESS) {
  ...
  if ((res = sxp_connect(&client, addresses->ai_addr, addresses->ai_addrlen)) != SXP_SUCCESS) {
    result = NET_ERROR;
    goto end_socket;
  }
...
netResult netio_serve(const char *port)
{
  ...
  if ((res = sxp_addrinfo_get(&addresses, NULL, port, &hints)) != SXP_SUCCESS) {
    result = res == SXP_TRY_AGAIN ? NET_TRY_AGAIN : NET_ERROR;
    goto end;
  }
  ...
  if ((res = sxp_bind(&server, addresses->ai_addr, addresses->ai_addrlen)) != SXP_SUCCESS) {
    result = NET_ERROR;
    goto end_socket;
  }
...
netResult netio_reset()
{
  if (netio_connections) {
    ...
    for (i = 0; i < netio_connection_count; i++) {
      if (netio_connection_active(i)) netio_connection_close(i);
    }
...
netResult netio_tick()
{
  ...
  if (netio_accepts_sockets && netio_active_count) {
    ...
    if (netio_poll_list[idx].revents & POLLIN) {
      sxp_t new_sock;
      if (sxp_accept(&netio_poll_list[idx].fd, &new_sock) == SXP_SUCCESS) {
  ...
  for (; event_count > 0 && idx < netio_active_count;) {
    ...
    for (conn = 0; conn < netio_connection_count; conn++)
      if (netio_connections[conn].socket == client->fd) break;
...
netResult netio_recv(connection_t *who, net_buffer_t *packet)
{
  ...
  for (; con < netio_connection_count; con++) {
    if (netio_connection_active(con)) {
      switch (packet_recv_packet(&(netio_connections[con].recv_buffer), packet)) {
        case PACKET_NOT_READY: continue;
        case PACKET_SUCCESS: *who = con; return NET_SUCCESS;
        case PACKET_ERROR:
        default: return NET_ERROR;
      }
    }
...
{
  if (who >= netio_connection_count) return 0;
...
{
  netio_connections =
      realloc(netio_connections, sizeof(netio_connections[0]) * (netio_connection_count + 1));
  if (!netio_connections) return NET_ERROR;

  memset(&netio_connections[netio_connection_count], 0, sizeof(netio_connections[0]));
  netio_connections[netio_connection_count].connection = netio_connection_count;
...
static netResult pull_data(struct netio_connection_info *connection)
{
  ...
  if (result == SXP_TRY_AGAIN) return NET_SUCCESS;
  return NET_ERROR;
}
...
static netResult push_data(struct netio_connection_info *connection)
{
  ...
  if (result == SXP_TRY_AGAIN) return NET_TRY_AGAIN;
  if (result != SXP_SUCCESS) return NET_ERROR;
  connection->send_buffer.size -= msg_size;
  memmove(connection->send_buffer.buffer, connection->send_buffer.buffer + msg_size,
